			...
			for (const validation in validations) {
				if (validation === 'passwordIsPassword') {
					...
					passwordRegExp.test(String(value))
						...
						: setPasswordErrorText(`Пароль должен содержать от 4 до 12 символов`);
					...
					nameRegExp.test(String(value))
						? setNameErrorText('')
						: setNameErrorText(`Никнейм должен содержать от 1 до 15 символов`);
				} else if (validation === 'messageIsMessage') {
					const messageRegExp = /^[\s|\S]{10,1000}$/;
					messageRegExp.test(String(value))
						? setMessageErrorText('')
						: setMessageErrorText(`Сообщение должно содержать от 10 до 1000 символов`);
				}
			}
		},
		[ validations, value ]
