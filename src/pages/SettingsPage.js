...
export const SettingsPage = () => {
    ...
    const { token, avatarURL, userEmail } = useSelector(
    ...
    useEffect(() => {
        ;[musicSlider.current, soundSlider.current, wordSlider.current].forEach(
            (elem) => {
                elem.addEventListener('mouseup', async (event) => {
                    if (
                        ...
                        return showMessage(
                            'Этот ползунок часто заедает! Попробуйте аккуратно его перетянуть :)'
                        )
                    }
                    dispatch(
                        setSettings(
                    ...
                    if (!token)
                        return showMessage(
                            'Рекомендуем авторизоваться, тогда настройки будут всегда с вами :)',
                            200
                        )
    ...
    const handleSwitch = useCallback(
        async (event) => {
            ...
            const { text, code } = await dispatch(
                postSettings(event.target.name, event.target.checked, token)
            )
            showMessage(text, code)
        },
        [dispatch, showMessage, token]
    ...
    const changeAvatar = async (event) => {
        ...
        const { text, code } = await dispatch(
            uploadAvatar(event.target.files[0], token)
        )
        showMessage(text, code)
    }
    ...
    const changeScreenshot = async (event) => {
        ...
        reader.onerror = function () {
            showMessage(
                'Произошла какая-то ошибка с добавлением скриншота',
                404
            )
        }
    ...
    const sendMessage = async (event) => {
        ...
        if ((!email.isDirty && !email.value) || !message.isDirty) {
            return showMessage(
                'Поля электронной почты и сообщения обязательны для заполнения'
            )
        }
        ...
        try {
            ...
            showMessage(
                'Возникла какая-то ошибка с отправлением вашей обратной связи. Попробуйте позже :)',
                404
            )
        }
    }
    ...
    const handleTheme = async (event) => {
        ...
        if (!token)
            return showMessage(
                'Рекомендуем авторизоваться, тогда выбранная тема будет всегда с вами :)',
                200
            )
        const { text, code } = await dispatch(
            postSettings('theme', newTheme, token)
        )
        showMessage(text, code)
    }
    ...
    const changeName = useCallback(
        async (event) => {
            ...
            if (!token)
                return showMessage(
                    'Для изменения никнейма необходимо авторизоваться',
                    404
                )
            if (!name.value)
                return showMessage(
                    'Вы ничего не ввели. Никнейм не изменён',
                    404
                )
                ...
                <div className={classes.card}>
                    <div className={classes.cardItem}>
                        ...
                        <SettingsSlider
                            ...
                            theme={theme}
                        />
                        <h6 className={classes.subtitle1}>
                    ...
                    <div className={classes.cardItem}>
                        ...
                        >
                            ...
                            {email.isDirty && email.emailErrorText ? (
                                ...
                                <span className={classes.info}>
                                    Адрес, на который мы обязательно вам ответим
                                </span>
                            )}
                            <textarea
                            ...
                            />
                            {message.isDirty && message.messageErrorText && (
                                <span
                                    style={{ color: 'red', fontWeight: 'bold' }}
                                    className={classes.info}
                                >
