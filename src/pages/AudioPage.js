...
export const AudioPage = () => {
    const { soundVolume, wordVolume, theme } = useSelector(
    ...
    const audioFail2 = useMemo(
        () => createSound(failSong2, soundVolume),
        [soundVolume]
    )
    const audioWord = useMemo(
        () => createSound(`${originURL}/${currentWord.audio}`, wordVolume),
    ...
    const answer = useCallback(
        (word) => {
            ...
            if (word === currentWord.word) {
                ...
                const correctButton = four.current.find(
                    (button) => button.value === word
                )
                ...
                seriesContainer.current.innerHTML = ''
                const correctButton = four.current.find(
                    (button) => button.value === currentWord.word
                )
                const failButton = four.current.find(
                    (button) => button.value === word
                )
                correctButton.className = classes.correctButton
    ...
    useEffect(() => {
        ...
        const keyboardClick = (event) => {
            ...
            let elem
            if (
                event.keyCode === fourKeyCode.top1 ||
                event.keyCode === fourKeyCode.num1
            ) {
                elem = four.current[0]
            } else if (
                event.keyCode === fourKeyCode.top2 ||
                event.keyCode === fourKeyCode.num2
            ) {
                elem = four.current[1]
            } else if (
                event.keyCode === fourKeyCode.top3 ||
                event.keyCode === fourKeyCode.num3
            ) {
                elem = four.current[2]
            } else if (
                event.keyCode === fourKeyCode.top4 ||
                event.keyCode === fourKeyCode.num4
            ) {
                elem = four.current[3]
            }
            answer(elem.value)
