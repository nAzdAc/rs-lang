    ...
    const answer = useCallback(
        (src) => {
            ...
            if (src === currentWord.image) {
                ...
                const goodOverlay = four.current.find(
                    (elem) => elem.dataset.name === currentWord.image
                )
    ...
    useEffect(() => {
        if (wordsArray.length && currentNumber < wordsArray.length) {
            const arr = wordsArray
                .filter((word) => word.image !== currentWord.image)
        ...
        <div className={classes.root}>
            {endGame ? (
                ...
                <div ref={gameBoard} className={classes.gameContainer}>
                    ...
                    <h3 className={classes.currentWord}>{`${
                        currentWord.word || ''
                    }`}</h3>
                    <p className={classes.example}>
                        {`${convertText(currentWord.textExample)}` || ''}
                    </p>
