...
export const GameStats = ({
    ...
    const audioWin = useMemo(
        () => createSound(winSong, soundVolume),
        [soundVolume]
    )
    const audioDefeat = useMemo(
        () => createSound(defeatSong, soundVolume),
        [soundVolume]
    )
    ...
    const sendUserStats = useCallback(async () => {
        ...
        const { text, code } = await dispatch(
            postStats(token, gameName, correctAnswers, failAnswers, allSeries)
        )
                        ...
                        <Table className={classes.table}>
                            ...
                            <TableBody>
                                {failAnswers.map((answer, index) => {
                                    return (
                                        <GameRow
                                            key={`${answer.word}fail-GameStats`}
                                            color="#FF001E"
                                            answer={answer}
                                        />
                                    )
                                ...
                                {correctAnswers.map((answer, index) => {
                                    return (
                                        <GameRow
                                            key={`${answer.word}correct-GameStats`}
                                            color="#28FC03"
                                            answer={answer}
                                        />
                                    )
